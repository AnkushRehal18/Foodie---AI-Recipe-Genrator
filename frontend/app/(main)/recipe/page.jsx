...
import { getOrGenerateRecipe, removeRecipeFromCollection, saveRecipeToCollection } from "@/actions/recipe.action";
...
function RecipeContent() {
  ...
  const handleToggleSave = async () => {
    if (!recipeId) {
      toast.error("Recipe could not be saved to the database. Please ensure the backend is running and refresh the page.");
      return;
    }
          ...
          <div className="bg-white p-8 md:p-10 border-2 border-stone-200 mb-6">
            ...
            <p className="text-lg text-stone-600 mb-6 font-light">
              {Array.isArray(recipe.description)
                ? recipe.description.map(block => block.children?.map(c => c.text).join("")).join(" ")
                : recipe.description}
            </p>
            ...
            <div className="flex flex-wrap gap-6 text-stone-600 mb-6">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">
                  {parseInt(recipe.prepTime) + parseInt(recipe.cookTime || recipe.cookTIme)} mins
                  total
                </span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="flex flex-wrap gap-3">
              <Button
                ...
                disabled={saving || removing || !recipeId}
                title={!recipeId ? "Backend is offline — recipe not saved to database" : ""}
                className={`${
                  isSaved
                    ? "bg-green-600 hover:bg-green-700 border-2 border-green-700"
                    : !recipeId
                    ? "bg-stone-400 border-2 border-stone-500 cursor-not-allowed"
